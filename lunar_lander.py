...
class LunarLander(gym.Env):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if(event.key == 275):
                    ...
                    # pygame.display.flip()
                    # self.clock.tick(self.fps)
                    # print('up key pressed', event.key)
                    # self.h=[0,0,1,0]    # Discrete
                    self.h=1    # Continuous
                    return 'upkeydown'
                elif(event.key == 274):
                    # pygame.display.flip()
    ...
    def step(self, action):
        ...
        return np.array(state), np.array([self.h, reward]), done, {}
...
if __name__=="__main__":
    ...
    while True:
        ...
        if steps % 20 == 0 or done:
            ...
            print("step {0} total_reward {1}".format(steps, total_reward))
        steps += 1
        if done: break
